...
class AdminDashboardScreenEpic extends StatefulWidget {
  const AdminDashboardScreenEpic({Key? key}) : super(key: key);

  @override
  State<AdminDashboardScreenEpic> createState() => _AdminDashboardScreenEpicState();
}

class _AdminDashboardScreenEpicState extends State<AdminDashboardScreenEpic>
  ...
  Future<void> _cargarPagosPendientes() async {
    ...
    try {
      ...
      String url = 'https://gallerappback-production.up.railway.app/api/v1/admin/pagos/pendientes';
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          _pagosPendientes = List<Map<String, dynamic>>.from(data);
          _isLoadingPagos = false;
        });
        print('✅ [ADMIN-EPIC-TAB2] Pagos cargados: ${_pagosPendientes.length}');
      } else {
        throw Exception('Error: ${response.statusCode}');
      }
    } catch (e) {
    ...
    return Card(
      ...
      child: ExpansionTile(
        leading: CircleAvatar(
          backgroundColor: usuario['is_premium'] ? Colors.gold : AppColors.primary,
          child: Text(
